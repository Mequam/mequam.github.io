...
/*
	this is an intentonaly global variable inteanded for use
	any where within the application that needs to reference the version of the app
*/
let CLUB_APP_CURRENT_VERSION = "1.0.0";

//this function creates a bootstrap warning element
function createWarningDiv(warningTxt) {
	let div = document.createElement("div");
	div.className = "alert alert-warning";
	
	let p = document.createElement("p");
	p.innerHTML = "Warning: you are only allowed to carry 14 clubs in your golf bag in match play competition";
...
function updateClubs(jsonStrClubs) {	
	localStorage.setItem("clubsUndo",loadClubDistancesStr());
	localStorage.setItem("clubs",jsonStrClubs);
}
//this function returns the user to the previous page FROM THE SAME SITE that they originated on
//E.G. the user will not end up outside of the domain they started on
function goBack() {
	document.location = document.referrer;
}

//this function returns the json string for the clubs array in a safe manner
function loadClubDistancesStr() {
	let clubs;
	// if "clubs" array already exists, load it from local storage
	if (localStorage.getItem("clubs")) {
		clubs = localStorage.getItem("clubs");
	}
	// otherwise create new "clubs" array, using resetAllClubs without re-load
	else {
		resetAllClubDistNoReload();	
		clubs = localStorage.getItem("clubs");
	}
	return clubs;
...
//this function synccs the display of the distance list to match the clubs list
function syncDistanceList() { 
       	/*
		we could change the actual contents of the table in the html file to match the list,
		but this solution is quick AND uses existing code.
	*/
	//by forcing a page reload we force a re-syncing of the list
	window.location.href = "clubDistanceList.html"; 
}
// replace the current "clubs" array with the previous one
function undoLastShot() {
	//store the clubsUndo into clubs
	localStorage.setItem("clubs",localStorage.getItem("clubsUndo"));
	syncDistanceList(clubs);
}

//resets all club distances without re-syncing the screen
function resetAllClubDistNoReload() {
